...
export default function ManageMeals(props) {
    ...
    const [manageMeals, setManageMeals] = useState([]);
    ...
    useEffect(() => {
        fetch("http://localhost:3002/services/" + flightNo.current).then(res => res.json()).then(json => {
            objt = json;
            setManageMeals(json.specialMeal);
        });
    }, []);
    ...
    const handleRowDelete = (oldData, resolve) => {
        ...
        let dataDelete = [...manageMeals];
        const index = oldData.tableData.id;
        dataDelete.splice(index, 1);
        setManageMeals([...dataDelete]);
        objt.specialMeal = dataDelete;
    ...
    const handleRowUpdate = (newData, oldData, resolve) => {
        ...
        if (errorList.length < 1) {
            let dataUpdate = [...manageMeals];
            const index = oldData.tableData.id;
            dataUpdate[index] = newData;
            setManageMeals(dataUpdate);
            objt.specialMeal = dataUpdate;
            UserService.updateServicesDetails(objt, flightNo.current);
            resolve()
        } else {
    ...
    const handleRowAdd = (newData, resolve) => {
        let errorList = []
        if (newData.name === undefined) {
            errorList.push("Please enter Meal name")
        }

        if (errorList.length < 1) { //no error
            let dataToAdd = [...manageMeals];
            dataToAdd.push(newData);
            setManageMeals(dataToAdd);
            objt.specialMeal = dataToAdd;
            UserService.updateServicesDetails(objt, flightNo.current);
            resolve();
        } else {
            setErrorMessages(errorList)
