...
export default function ShoppingItems(props) {
    ...
    const [inflightShop, setInflightShop] = useState([]);
    ...
    useEffect(() => {
        fetch("http://localhost:3002/services/" + flightNo.current).then(res => res.json()).then(json => {
            objt = json;
            setInflightShop(json.inflightShop);
        });
    }, []);
    ...
    const handleRowDelete = (oldData, resolve) => {
        ...
        let dataDelete = [...inflightShop];
        const index = oldData.tableData.id;
        dataDelete.splice(index, 1);
        setInflightShop([...dataDelete]);
        objt.inflightShop = dataDelete;
    ...
    const handleRowUpdate = (newData, oldData, resolve) => {
        ...
        if (errorList.length < 1) {
            let dataUpdate = [...inflightShop];
            const index = oldData.tableData.id;
            dataUpdate[index] = newData;
            setInflightShop(dataUpdate);
            objt.inflightShop = dataUpdate;
            UserService.updateServicesDetails(objt, flightNo.current);
            resolve()
        } else {
    ...
    const handleRowAdd = (newData, resolve) => {
        ...
        if (errorList.length < 1) { //no error
            let dataToAdd = [...inflightShop];
            dataToAdd.push(newData);
            setInflightShop(dataToAdd);
            objt.inflightShop = dataToAdd;
            UserService.updateServicesDetails(objt, flightNo.current);
            resolve();
        } else {
            setErrorMessages(errorList)
    ...
    return (
        <div>
            ...
            <MaterialTable
                columns={columns}
                title={'InFlight Shopping Items Services'}
                icons={tableIcons}
                options={{
                    paging: false,
                    search: false,
